/**
 * H1b — facing for own machines only. Runs every matchup specified in
 * docs/heuristics.md §H1b and writes results.md beside this file.
 *
 *   node --import tsx experiments/h1b-own-facing/run.ts
 *   PAIRS_SCALE=0.05 node --import tsx experiments/h1b-own-facing/run.ts   # smoke test
 *
 ...
 *
 * The controls matter here: H1's published numbers were measured under the
 * previous deployment rule, so `heuristic-facing` vs `heuristic` and
 * `heuristic` vs `greedy` are re-measured in this same run rather than compared
 * across the change.
 */
...
const MATCHUPS: [string, string, string, string, number][] = [
  ["Dropping the enemy half (primary)", "heuristic-facing-own", "heuristic-facing", "plains-and-forests", 100],
  ["Against plain heuristic", "heuristic-facing-own", "heuristic", "plains-and-forests", 100],
  ["H1 again, under this code", "heuristic-facing", "heuristic", "plains-and-forests", 100],
  ["Against greedy — new", "heuristic-facing-own", "greedy", "plains-and-forests", 100],
  ["Against greedy — control", "heuristic", "greedy", "plains-and-forests", 100],
  ["Against plain heuristic, Mountains", "heuristic-facing-own", "heuristic", "mountains", 50],
  ["Against plain heuristic, Coastal", "heuristic-facing-own", "heuristic", "coastal", 50],
];
...
const lines: string[] = [];
lines.push(`# H1b results`, ``);
lines.push(`Generated by \`experiments/h1b-own-facing/run.ts\` at commit \`${sha}\`${dirty}, ${new Date().toISOString().slice(0, 10)}.`);
...
console.log(`\nwrote experiments/h1b-own-facing/results.md`);
