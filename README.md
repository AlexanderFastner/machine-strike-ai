...
and **Chasms** — generated by `tools/gen_boards.py`. Flat is the baseline for combat maths and sits out the
arena's testing rotation: with no terrain there is no position worth taking, so it adds games without adding
tactics. Every one is **180° rotationally symmetric**, so both
