...
**[C] Board library — built.** `packages/data/boards/` holds nine boards: **Flat** (all grassland, the baseline
for testing combat maths), **Plains and Forests**, **Coastal**, **River Valley**, **Split Peaks**, **Mountains**,
**Caldera**, **Chasms** and **Badlands**. All but Flat are generated by `tools/gen_boards.py`, which authors only
the top half and derives the bottom, so 180° symmetry is guaranteed by construction rather than by eye. It also
asserts that no chasm sits in a deployment row, and that no arrangement of chasms fences off a tile a
non-flying machine could otherwise stand on.

**[C] Map editor — built.** Players can draw their own boards, with mirror-editing on by default so custom
boards inherit the same fairness property. Custom boards persist in browser storage and can be exported as JSON.

**Still deferred:**

1. **Random mirrored generator** — procedurally generated boards. Needs authored constraints on terrain mix,
   not just a symmetry filter, or it will produce unplayable boards.
...
- [ ] §2.1 Tune the random generator's terrain constraints (the board library itself is built).
