...
**[C] Board library — built.** `packages/data/boards/` holds six boards: **Flat** (all grassland, the baseline
for testing combat maths), **Plains and Forests**, **Coastal**, **Split Peaks**, **Mountains** and **Chasms**.
All are generated by `tools/gen_boards.py`, which authors only the top half and derives the bottom, so 180°
symmetry is guaranteed by construction rather than by eye. It also asserts no chasm sits in a deployment row.
