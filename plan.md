...
| Phase | Deliverable | Done when |
|---|---|---|
| **4. Arena** | Headless runner, Random + Greedy, `AgentPlayer` in the UI | ✅ **Done.** `npm run arena` runs matches and tournaments with paired games, a fixed draft book and Bradley–Terry Elo with error bars; seven agents ranked. See [docs/arena.md](docs/arena.md). **Play vs AI** in the web app picks any of them and one of the draft-book sets |
| **4b. Sets and positions** | Results store, set sweeps, agent-chosen deployment, then **evolutionary search** over arrangements and sets | 🔄 **Store, sweeps and chosen deployment done** — [docs/results.md](docs/results.md), [docs/arena.md](docs/arena.md#choosing-where-to-start). Remaining: the search itself |
